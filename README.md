...
- ゲームのマニュアル：[README_PLAY.md](./README_Game.md)  
- ゲーム本編（実行ファイル）のダウンロード：[Releases](https://github.com/AglaoDev-jp/majinnosumu_tower-Ren-Py-Remake/releases/download/v1/majinnosumu_tower_Remake_v1-1.0-win.zip) 
...
- **Ren’Py Remake v1**: 2026年1月16日 ~ 2026年2月15日 
...
- **クレジット表記**:
  ...
  - 表記例:  
    ...
    ```
- **ライセンスファイルの同梱**:
  - `OFL.txt`をゲームパッケージの適切な場所（例: `./licenses/`フォルダ）に含めてください。  
...
### ver-5.0 → Ren’Py Remake v1 の改善点
- 一部シナリオの誤字、脱字を修正しました。
- エンドクレジットを修正しました。
- エンドロール中に自動でシェードを行うようにしました。
- 画像の一部を再度ChatGPTで生成しなおして変更しました。（dark_corridor_2.png 鏡の勇者 王女の画像全般）
- 画像を追加しました。（オープニング画像、あとがき画像）
- 文字を縁取りしました。
- フルボイス化しました。
- エンディングリスト、ヒント、あとがきを実装しました。
- オープニング画面のタイトルを画像にしました。
- タイトルのシェード、アニメーションを実装しました。
- オープニング画面の項目で、以下の設定をしました。
  - ベストエンドクリアで
    - エンディングリスト、ヒントのアンロック
  - エンディングをすべてクリアすると
    - オープニング画像が変更される
    - あとがきのアンロック
- 音源などのアセットを、Ren’Py標準の アーカイブ化（.rpa）でまとめました。
...
## 問題点　改善点
- 文字送りアイコンは実装しましたが、ページめくりアイコンの実装ができていません。
- 文字送りアイコンの位置を文末にすることができていません。
- 旧バージョンにあった、テキストへのシェード機能は未実装です。
