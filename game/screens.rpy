...
screen say(who, what):
    ...
    if _window:
        ...
        text what:
            ...
            # 左寄せ
            textalign 0.0
            size 48          # ← 文字サイズを追加（デフォルトはだいたい28前後）
...
screen main_menu():
    ...
    if all(e in clears for e in ["A", "B", "C"]):
        add "demon_tower_all_endings.png" xysize (config.screen_width, config.screen_height)
    else:
    ...
    add "images/logo_majin_tower.png":
        xalign 0.5         # 横中央
        yalign 0.22        # 縦位置（0.15〜0.25くらいで調整が効く）
        zoom 0.80          # ロゴだけフェードイン（不要なら削除）0.75〜0.85で好みに調整
        at title_logo_fade_in
...
screen file_slots(title):
    ...
    use game_menu(title):
        ...
        fixed:
            ...
            ## ページ名。クリックすると編集できるように、ボタンとして表示して
            ## います。
            button:
                style "page_label"
...
screen history():
    ...
    # ★ vpgrid をやめて viewport 固定にする（高さ事故を根絶）
    use game_menu(_("ヒストリー"), scroll="viewport", yinitial=1.0, spacing=gui.history_spacing):

        # style_prefix は残してOK（色やフォントなど他の見た目設定が効く）
...
style history_text:
    line_spacing 6
    # ↓ これらは “積みレイアウト” を壊しやすいので外す（特に ypos）
...
## Help（ヘルプ）スクリーン ##############################################################
##
## キーやマウスの割り当てに関する情報を表示するスクリーン。実際のヘルプは他の
## スクリーン（keyboard_help、mouse_help、gamepad_help）を使います。
...
## バブルスクリーン ####################################################################
##
## バブルスクリーンは、吹き出しを使用する際に、プレイヤーにダイアログを表示
## するために使用されます。バブルスクリーンはSayスクリーンと同じパラメータを持
## ち、idが "what "のdisplayableを作成しなければならず、idが "namebox", "who",
## "window "のdisplayableを作成できます。
##
...
screen hint_screen():
    ...
    frame:
        ...
        vbox:
            ...
            序盤に現れる**二手に分かれる道**で、
            "１．右へ進む。" を選ぶと、→ 大きなホールへと辿り着きます。
              → そこで、選択肢の "４．隣の部屋に行ってみる。" を選ぶと`Cシナリオ`に分岐します。
              → そのままホールの謎を解くと`Aシナリオ`です。

            "２．左へ進む。" を選ぶと、 → `Bシナリオ`です。                       
            """ size 28
...
# ------------------------------------------------------------
# あとがき画面（メインメニュー用）
# ・文章が長くなるので viewport でスクロール対応
# ・mousewheel True：マウスホイールでスクロール
# ・draggable True：ドラッグでもスクロール（タッチパッド勢にも優しい）
# ------------------------------------------------------------
screen afterword_screen():
    tag menu

    add Transform("images/ending_afterword_view.png", size=(config.screen_width, config.screen_height))

    frame:
        background Solid("#0008")  # 黒＋半透明
        xalign 0.5
        yalign 0.5
        xsize 0.8
        ysize 0.7
        ...
        vbox:
            ...
            # タイトル
            text "あとがき" size 48 color "#FFFFFF" # 明るく

            # ------------------------------
            # ここが本体：スクロール領域
            # ------------------------------
            viewport:
                # 表示領域の高さ（フレーム内でボタン分を残す）
                ysize 420
                ...
                vbox:
                    ...
                    # あとがき本文（長文OK）
                    # ※ Ren'Pyのtextは、改行を""" ... """ の複数行文字列でそのまま扱えるそうです。
                    text """
